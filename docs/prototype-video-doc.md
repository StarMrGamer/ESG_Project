...
- **The call list is TEN rows, not eleven.** The previous draft said eleven. That was the
  N/M/K frozen on 2026-08-24; the deep+quality sweep re-froze it at **13 / 10 / 1** on run
  `1fc384a2fa92499d`, which is the run the whitepaper, the slide and the Sepolia anchor all
  quote. Say **ten**.
- **There are TWO elevens in this pitch and only one of them moved.** The call list is 10.
  *"Eleven of the seventeen have never issued a labelled green bond"* is **correct** and stays.
  Do not let the two swap places under pressure.
- Every figure below was re-checked against the engine on 2026-08-26.

---

## 1. What the video is

Not a feature tour. It is **the analyst's Monday morning, recorded** — the character the stage
just introduced, doing the sequence the stage promised, ending on the proof the stage will build
on.

Design rules that follow:

1. **One continuous flow, in her order.** Features appear because the flow reaches them, never
   as a menu.
2. **One company for §§5–7.** Use **RHB Bank Bhd** — see §4 below for why it is the only
   correct choice.
3. **Fully narrated.** Presenter silent during playback. Narration recorded first, picture cut
   to it.
4. **Honest on camera.** MOCK / PROVISIONAL badges stay visible. Hiding one contradicts beat 7's
   "we say the gap before you find it."
5. **The interrogation is not optional.** The product's own sentence is *"an AI that
   **interrogates** along ESG-native axes, **then competes**."* A video that opens on a board and
   never asks a question has shown one of three beats. §3 exists for this reason.
...
## 2. Why RHB Bank Bhd is the company for §§5–7

It is the only name in the basket that demonstrates every feature in one click path:

| | |
|---|---|
| Hidden Winner | disagreement **+0.804**, confidence **0.779**, **14** dated signals |
| on the call list | it is in the **M** bucket — §4 and §5 are the same company |
| contested governance | **8 up, 1 down** — the honest moment in §6 |
| a real near-term catalyst | green-bond framework, Sustainalytics SPO, **never issued under it** |
| real incumbent rating | resolves live on LSEG |
| real headlines | three dated, sourced |

It also crossed into Hidden Winners **on more evidence, with no threshold moved** — which is the
story beat 5 tells. Same company, same run, one narrative.

---

## 3. The linkage map

| Stage (before video) | Video delivers | Stage (after) builds on it |
|---|---|---|
| "built for one person — the analyst" | her, doing the whole flow | "why can **she** trust the number" |
| "your quadrants, rendered live" | §1 the living matrix | — (never re-explained) |
| "filtered by risk appetite" | §2 the tier flip | "set by her fund" answers the CIO question |
| "it interrogates, then competes" | **§3 the interrogation** | the concept sentence is now shown, not claimed |
| — | §4 the call list, seen | "eleven of your seventeen…" lands on a list judges saw |
| "a decision, not a score" | §5 two reads unmerged | never needs repeating |
| — | §6 three clicks to source | "the AI never computes the score" becomes believable |
| — | §7 verify + tamper | "nobody else in this category can do" · "cryptographic" |
| — | §7+ Adaro | beat 7's "we say the gap before you find it" is pre-paid |

**The discipline:** stage explains, video shows, slides remember. If a sentence appears in two of
the three, delete it from the smaller one.

---

## 4. Production checklist

- [ ] **Decide the stage length first** — 2:30 video and trim 30s of speaking, or run 2:00 on stage
- [ ] Screen-record 1080p, cursor visible, browser chrome hidden, real app
- [ ] **Warm every company you will show** — a Compete runs a live model call, and an unwarmed
      one means dead air. RHB and any §3 comparison company, warmed before you hit record
- [ ] **Restart the server before recording.** Python changes do not hot-reload; the frontend does
- [ ] Record narration first, cut picture to it
- [ ] No music under narration, or barely audible
- [ ] MOCK / PROVISIONAL badges left visible
- [ ] Tamper toggle rehearsed — no fumbling, no cursor hunting
- [ ] End-card URL = slide 4 URL = the URL said on stage
- [ ] Export a backup to a second device

## 5. Measured narration budget
